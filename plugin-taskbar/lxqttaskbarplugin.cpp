...
LXQtTaskBarPlugin::LXQtTaskBarPlugin(
    const ILXQtPanelPluginStartupInfo &startupInfo)
    : QObject(), ILXQtPanelPlugin(startupInfo)

{
  mTaskBar = new LXQtTaskBar(this);
  qInfo() << "called from task manager .." << endl;
}

LXQtTaskBarPlugin::~LXQtTaskBarPlugin() { delete mTaskBar; }

QDialog *LXQtTaskBarPlugin::configureDialog() {
  return new LXQtTaskbarConfiguration(settings());
}

void LXQtTaskBarPlugin::realign() { mTaskBar->realign(); }
