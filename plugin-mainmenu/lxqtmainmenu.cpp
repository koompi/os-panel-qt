...
LXQtMainMenu::LXQtMainMenu(const ILXQtPanelPluginStartupInfo &startupInfo)
    : QObject(), ILXQtPanelPlugin(startupInfo), mMenu(0),
      mShortcut(0), mSearchEditAction{new QWidgetAction{this}},
      mSearchViewAction{new QWidgetAction{this}}, mMakeDirtyAction{new QAction{
                                                      this}},
      mFilterMenu(true), mFilterShow(true), mFilterClear(false),
      mFilterShowHideMenu(true), mHeavyMenuChanges(false) {
#ifdef HAVE_MENU_CACHE
  mMenuCache = nullptr;
  mMenuCacheNotify = nullptr;
#endif
  qInfo() << "called from Applicaiton menu" << endl;
  ...
  // 2. delaying of installEventFilter because in c-tor mButton has no parent
  // widget
  ...
  connect(mSearchView, &QAbstractItemView::activated, this,
          &LXQtMainMenu::showHideMenu);
  ...
  connect(mSearchEdit, &QLineEdit::textChanged,
          [this](QString const &) { mSearchTimer.start(); });
  ...
  mShortcut = GlobalKeyShortcut::Client::instance()->addAction(
      QString{}, QStringLiteral("/panel/%1/show_hide").arg(settings()->group()),
      LXQtMainMenu::tr("Show/hide main menu"), this);
  if (mShortcut) {
    connect(mShortcut, &GlobalKeyShortcut::Action::registrationFinished,
            [this] {
...
void LXQtMainMenu::showMenu() {
  ...
  if (mFilterMenu || mFilterShow) {
    // Note: part of the workadound for
...
void LXQtMainMenu::settingsChanged() {
  ...
  if (settings()->value(QStringLiteral("showText"), false).toBool()) {
    mButton.setText(settings()
                        ->value(QStringLiteral("text"), QStringLiteral("Start"))
                        .toString());
  ...
  QString menu_file =
      settings()->value(QStringLiteral("menu_file"), QString()).toString();
  ...
  if (mMenuFile != menu_file) {
    ...
    mMenuCacheNotify = menu_cache_add_reload_notify(
        mMenuCache, (MenuCacheReloadNotify)menuCacheReloadNotify, this);
#else
    mXdgMenu.setEnvironments(QStringList() << QStringLiteral("X-LXQT")
                                           << QStringLiteral("LXQt"));
    ...
    if (res) {
      ...
      QMessageBox::warning(0, QStringLiteral("Parse error"),
                           mXdgMenu.errorString());
      return;
    }
#endif
  }
  ...
  mFilterClear =
      settings()->value(QStringLiteral("filterClear"), false).toBool();
  mFilterShowHideMenu =
      settings()->value(QStringLiteral("filterShowHideMenu"), true).toBool();
  if (mMenu) {
  ...
  mSearchView->setMaxItemsToShow(
      settings()->value(QStringLiteral("filterShowMaxItems"), 10).toInt());
  mSearchView->setMaxItemWidth(
      settings()->value(QStringLiteral("filterShowMaxWidth"), 300).toInt());
...
void LXQtMainMenu::searchMenu() {
  ...
  if (mFilterShow) {
    ...
    // TODO: how to force the menu to recalculate it's size in a more elegant
    // way?
...
static void menuInstallEventFilter(QMenu *menu, QObject *watcher) {
  for (auto const &action :
       const_cast<QList<QAction *> const &&>(menu->actions())) {
    if (action->menu())
...
void LXQtMainMenu::buildMenu() {
  ...
  connect(mMenu, &QMenu::aboutToHide, &mHideTimer,
  ...
  // Note: setting readOnly to true to avoid wake-ups upon the Qt's internal
  // "blink" cursor timer (if the readOnly is not set, the "blink" timer is
  // active also in case the menu is not shown ->
...
void LXQtMainMenu::setMenuFontSize() {
  ...
  bool customFont =
      settings()->value(QStringLiteral("customFont"), false).toBool();

  if (customFont) {
    menuFont = mMenu->font();
    menuFont.setPointSize(
        settings()->value(QStringLiteral("customFontSize")).toInt());
  }
        ...
        QStringLiteral("#MainMenu { qproperty-icon: url(%1); }")
            .arg(settings()
                     ->value(QLatin1String("icon"),
                             QLatin1String(LXQT_GRAPHICS_DIR "/helix.svg"))
                     .toString()));
  } else {
...
QDialog *LXQtMainMenu::configureDialog() {
  return new LXQtMainMenuConfiguration(settings(), mShortcut,
                                       QStringLiteral(DEFAULT_SHORTCUT));
}
/************************************************
...
struct MatchAction {
  ...
  bool operator()(QAction *action) {
    return action->text().startsWith(key_, Qt::CaseInsensitive);
  }
  QString key_;
};
  ...
  if (obj == mButton.parentWidget()) {
    ...
    if (event->type() == QEvent::KeyPress) {
      ...
      if (keyEvent->modifiers() & ~Qt::ShiftModifier) {
        ...
        return true;
      } else // go to the menu item which starts with the pressed key if there
             // is an active action.
      {
        ...
        if (action != 0) {
          ...
          QList<QAction *>::iterator it =
              qFind(actions.begin(), actions.end(), action);
    ...
    if (obj == mMenu) {
      if (event->type() == QEvent::Resize) {
        ...
        if (e->oldSize().isValid() && e->oldSize() != e->size()) {
          qInfo() << "called popup window" << endl;
