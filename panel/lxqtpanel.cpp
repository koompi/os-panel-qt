...
/************************************************
 ...
 String is one of "Top", "Left", "Bottom", "Right", string is not case
 sensitive. If the string is not correct, returns defaultValue.
 ************************************************/
ILXQtPanel::Position
LXQtPanel::strToPosition(const QString &str,
                         ILXQtPanel::Position defaultValue) {
 ...
 ************************************************/
LXQtPanel::LXQtPanel(const QString &configGroup, LXQt::Settings *settings,
                     QWidget *parent)
    : QFrame(parent), mSettings(settings),
      mConfigGroup(configGroup), mPlugins{nullptr},
      mStandaloneWindows{new WindowNotifier}, mPanelSize(0), mIconSize(0),
      mLineCount(0), mLength(0), mAlignment(AlignmentLeft),
      mPosition(ILXQtPanel::PositionBottom),
      mScreenNum(0), // whatever (avoid conditional on uninitialized value)
      mActualScreenNum(0), mHidable(false), mVisibleMargin(true),
      mHidden(false), mAnimationTime(0), mReserveSpace(true),
      mAnimation(nullptr), mLockPanel(false) {
  ...
  // changed. A window is always activated on mouse click. Please see the source
  // code of Qt5: src/plugins/platforms/xcb/qxcbwindow.cpp. void
  ...
  // panel 1 when two task managers are open Besides, this breaks minimizing or
  // restoring windows when clicking on the taskbar buttons. To workaround this
  // regression bug, we need to add this window flag here. However, since the
  // panel gets no keyboard focus, this may decrease accessibility since it's
  // not possible to use the panel with keyboards. We need to find a better
  // solution later.
  ...
  // Indicates that the widget should have a translucent background, i.e., any
  // non-opaque regions of the widgets will be translucent because the widget
  // will have an alpha channel. Setting this ...
  ...
  // LXQtPanel (inherits QFrame) -> lav (QGridLayout) -> LXQtPanelWidget
  // (QFrame) -> LXQtPanelLayout
  ...
  connect(mLayout, &LXQtPanelLayout::pluginMoved, this,
          &LXQtPanel::pluginMoved);
  ...
  connect(&mShowDelayTimer, &QTimer::timeout,
          [this] { showPanel(mAnimationTime > 0); });

  // screen updates
  connect(qApp, &QApplication::screenAdded, this, [this](QScreen *newScreen) {
    connect(newScreen, &QScreen::virtualGeometryChanged, this,
            &LXQtPanel::ensureVisible);
    connect(newScreen, &QScreen::geometryChanged, this,
            &LXQtPanel::ensureVisible);
    ensureVisible();
  });
  connect(qApp, &QApplication::screenRemoved, this, [this](QScreen *oldScreen) {
    disconnect(oldScreen, &QScreen::virtualGeometryChanged, this,
               &LXQtPanel::ensureVisible);
    disconnect(oldScreen, &QScreen::geometryChanged, this,
               &LXQtPanel::ensureVisible);
  ...
  connect(LXQt::Settings::globalSettings(), SIGNAL(settingsChanged()), this,
          SLOT(update()));
  connect(lxqtApp, SIGNAL(themeChanged()), this, SLOT(realign()));

  connect(mStandaloneWindows.data(), &WindowNotifier::firstShown,
          [this] { showPanel(true); });
  connect(mStandaloneWindows.data(), &WindowNotifier::lastHidden, this,
          &LXQtPanel::hidePanel);
  ...
  // NOTE: Some (X11) WMs may need the geometry to be set before
  // QWidget::show().
  setPanelGeometry();
...
void LXQtPanel::readSettings() {
  ...
  mHidable =
      mSettings->value(QStringLiteral(CFG_KEY_HIDABLE), mHidable).toBool();
  mHidden = mHidable;

  mVisibleMargin =
      mSettings->value(QStringLiteral(CFG_KEY_VISIBLE_MARGIN), mVisibleMargin)
          .toBool();

  mAnimationTime =
      mSettings->value(QStringLiteral(CFG_KEY_ANIMATION), mAnimationTime)
          .toInt();
  mShowDelayTimer.setInterval(mSettings
                                  ->value(QStringLiteral(CFG_KEY_SHOW_DELAY),
                                          mShowDelayTimer.interval())
                                  .toInt());

  // By default we are using size & count from theme.
  setPanelSize(
      mSettings->value(QStringLiteral(CFG_KEY_PANELSIZE), PANEL_DEFAULT_SIZE)
          .toInt(),
      false);
  setIconSize(
      mSettings
          ->value(QStringLiteral(CFG_KEY_ICONSIZE), PANEL_DEFAULT_ICON_SIZE)
          .toInt(),
      false);
  setLineCount(
      mSettings
          ->value(QStringLiteral(CFG_KEY_LINECNT), PANEL_DEFAULT_LINE_COUNT)
          .toInt(),
      false);
  ...
  setPosition(mScreenNum,
              strToPosition(
                  mSettings->value(QStringLiteral(CFG_KEY_POSITION)).toString(),
                  PositionBottom),
              false);

  setAlignment(
      Alignment(mSettings->value(QStringLiteral(CFG_KEY_ALIGNMENT), mAlignment)
                    .toInt()),
      false);

  QColor color = mSettings->value(QStringLiteral(CFG_KEY_FONTCOLOR), QString())
                     .value<QColor>();
  if (color.isValid())
    setFontColor(color, true);

  setOpacity(mSettings->value(QStringLiteral(CFG_KEY_OPACITY), 100).toInt(),
             true);
  mReserveSpace =
      mSettings->value(QStringLiteral(CFG_KEY_RESERVESPACE), true).toBool();
  color = mSettings->value(QStringLiteral(CFG_KEY_BACKGROUNDCOLOR), QString())
              .value<QColor>();
  if (color.isValid())
    setBackgroundColor(color, true);

  QString image =
      mSettings->value(QStringLiteral(CFG_KEY_BACKGROUNDIMAGE), QString())
          .toString();
  if (!image.isEmpty())
    setBackgroundImage(image, false);

  mLockPanel =
      mSettings->value(QStringLiteral(CFG_KEY_LOCKPANEL), false).toBool();
...
void LXQtPanel::saveSettings(bool later) {
  ...
  mSettings->setValue(QStringLiteral(CFG_KEY_POSITION),
                      positionToStr(mPosition));

  mSettings->setValue(QStringLiteral(CFG_KEY_ALIGNMENT), mAlignment);

  mSettings->setValue(QStringLiteral(CFG_KEY_FONTCOLOR),
                      mFontColor.isValid() ? mFontColor : QColor());
  ...
  mSettings->setValue(QStringLiteral(CFG_KEY_BACKGROUNDIMAGE),
                      QFileInfo(mBackgroundImage).exists() ? mBackgroundImage
                                                           : QString());
  ...
  mSettings->setValue(QStringLiteral(CFG_KEY_SHOW_DELAY),
                      mShowDelayTimer.interval());
...
QStringList pluginDesktopDirs() {
  QStringList dirs;
  dirs << QString::fromLocal8Bit(qgetenv("LXQT_PANEL_PLUGINS_DIR"))
              .split(QLatin1Char(':'), QString::SkipEmptyParts);
  dirs << QStringLiteral("%1/%2").arg(XdgDirs::dataHome(),
                                      QStringLiteral("/lxqt/lxqt-panel"));
  dirs << QStringLiteral(PLUGIN_DESKTOPS_DIR);
  qDebug() << "plugin directory: " << dirs << endl;
  return dirs;
}
...
void LXQtPanel::loadPlugins() {
  ...
  connect(mPlugins.data(), &PanelPluginsModel::pluginAdded, mLayout,
          &LXQtPanelLayout::addPlugin);
  connect(mPlugins.data(), &PanelPluginsModel::pluginMovedUp, mLayout,
          &LXQtPanelLayout::moveUpPlugin);
  // reemit signals
  connect(mPlugins.data(), &PanelPluginsModel::pluginAdded, this,
          &LXQtPanel::pluginAdded);
  connect(mPlugins.data(), &PanelPluginsModel::pluginRemoved, this,
          &LXQtPanel::pluginRemoved);

  const auto plugins = mPlugins->plugins();
  for (auto const &plugin : plugins) {
    mLayout->addPlugin(plugin);
    qInfo() << "this is lxqtpanel plugin looping." << endl;
    connect(plugin, &Plugin::dragLeft, [this] {
    ...
    case LXQtPanel::PositionBottom:
      KWindowSystem::setExtendedStrut(wid,
                                      ...
                                      /* Bottom */ wholeScreen.bottom() -
                                          rect.bottom() + getReserveDimension(),
                                      rect.left(), rect.right());
      break;
    ...
    case LXQtPanel::PositionRight:
      KWindowSystem::setExtendedStrut(wid,
                                      ...
                                      /* Right  */ wholeScreen.right() -
                                          rect.right() + getReserveDimension(),
                                      rect.top(), rect.bottom(),
                                      /* Top    */ 0, 0, 0,
                                      /* Bottom */ 0, 0, 0);
      break;
...
void LXQtPanel::showConfigDialog() {
  if (mConfigDialog.isNull())
    mConfigDialog =
        new ConfigPanelDialog(this, nullptr /*make it top level window*/);
...
void LXQtPanel::updateStyleSheet() {
  ...
  QStringList sheet;
  sheet << QStringLiteral("Plugin > QAbstractButton, LXQtTray { "
                          "qproperty-iconSize: %1px %1px; }")
               .arg(mIconSize);
  sheet << QStringLiteral("Plugin > * > QAbstractButton, TrayIcon { "
                          "qproperty-iconSize: %1px %1px; }")
               .arg(mIconSize);

  if (mFontColor.isValid())
    sheet << QString(QStringLiteral("Plugin * { color: ") + mFontColor.name() +
                     QStringLiteral("; }"));
  ...
  if (mBackgroundColor.isValid()) {
    ...
    sheet << QString(
        QStringLiteral(
            "LXQtPanel #BackgroundWidget { background-color: rgba(") +
  ...
  if (QFileInfo(mBackgroundImage).exists())
    sheet << QString(
        QStringLiteral(
            "LXQtPanel #BackgroundWidget { background-image: url('") +
        mBackgroundImage + QStringLiteral("');}"));
...
void LXQtPanel::setPosition(int screen, ILXQtPanel::Position position,
  ...
  // Qt 5 adds a new class QScreen and add API for setting the screen of a
  // QWindow. so we had better use it. However, without this, our program should
  // still work as long as XRandR is used. Since XRandR combined all screens
  // into a large virtual desktop every screen and their virtual siblings are
  // actually on the same virtual desktop. So things still work if we don't set
  // the screen correctly, but this is not the case for other backends, such as
  // the upcoming wayland support. Hence it's better to set it.
  if (windowHandle()) {
    ...
    // NOTE: When you move a window to another screen, Qt 5 might recreate the
    // window as needed But luckily, this never happen in XRandR, so Qt bug
    // #40681 is not triggered here. (The only exception is when the old screen
    // is destroyed, Qt always re-create the window and this corner case
    // triggers #40681.) When using other kind of multihead settings, such as
    // Xinerama, this might be different and unless Qt developers can fix their
    // bug, we have no way to workaround that.
...
bool LXQtPanel::event(QEvent *event) {
  ...
  case QEvent::WinIdChange: {
    ...
    // the winId() may be changed at runtime. So we need to reset all X11
    // properties when this happens.
    qDebug() << "WinIdChange" << hex << effectiveWinId() << "handle"
...
void LXQtPanel::showPopupMenu(Plugin *plugin) {
  ...
  /* Note: in multihead & multipanel setup the QMenu::popup/exec places the
   * window sometimes wrongly (it seems that this bug is somehow connected to
   * misinterpretation of QDesktopWidget::availableGeometry)
   */
...
QRect LXQtPanel::calculatePopupWindowPos(QPoint const &absolutePos,
  ...
  // NOTE: We cannot use AvailableGeometry() which returns the work area here
  // because when in a multihead setup with different resolutions. In this case,
  // the size of the work area is limited by the smallest monitor and may be
  // much smaller than the current screen and we will place the menu at the
  // wrong place. This is very bad for UX. So let's use the full size of the
  // screen.
 ...
 ************************************************/
QRect LXQtPanel::calculatePopupWindowPos(const ILXQtPanelPlugin *plugin,
                                         const QSize &windowSize) const {
  Plugin *panel_plugin = findPlugin(plugin);
  if (nullptr == panel_plugin) {
    qWarning() << Q_FUNC_INFO << "Wrong logic? Unable to find Plugin* for"
               << plugin << "known plugins follow...";
    const auto plugins = mPlugins->plugins();
    for (auto const &plug : plugins)
  ...
  // Note: assuming there are not contentMargins around the "BackgroundWidget"
  // (LXQtPanelWidget)
  return calculatePopupWindowPos(globalGeometry().topLeft() +
                                     panel_plugin->geometry().topLeft(),
                                 windowSize);
}
 ...
 ************************************************/
QString LXQtPanel::qssPosition() const { return positionToStr(position()); }
...
void LXQtPanel::pluginMoved(Plugin *plug) {
  ...
  for (int i = 0; i < mLayout->count(); ++i) {
    ...
    if (plugin) {
      if (found) {
        // we found our plugin in previous cycle -> is before this (or empty as
        // last)
...
void LXQtPanel::userRequestForDeletion() {
  ...
  const QStringList plugins =
      mSettings->value(QStringLiteral("plugins")).toStringList();
  mSettings->endGroup();
...
bool LXQtPanel::isPluginSingletonAndRunnig(QString const &pluginId) const {
  ...
  if (nullptr == plugin)
    ...
    return plugin->iPlugin()->flags().testFlag(
        ILXQtPanelPlugin::SingleInstance);
}
