...
public:
  ...
  bool isHorizontal() const {
    return position() == PositionBottom || position() == PositionTop;
  }
   ...
   * popup window with size wgetReserveDimensionindowSize.
   ...
   */
  virtual QRect calculatePopupWindowPos(const QPoint &absolutePos,
                                        const QSize &windowSize) const = 0;
  /**
   * @brief Helper method for calculating the global screen position of a
   ...
   * @return The global screen position where the popup window can be shown.
   */
  virtual QRect calculatePopupWindowPos(const ILXQtPanelPlugin *plugin,
                                        const QSize &windowSize) const = 0;

  /*!
   * \brief By calling this function, a plugin (or any other object) notifies
   * the panel about showing a (standalone) window/menu -> the panel needs this
   * to avoid "hiding" in case any standalone window is shown. The widget/window
   * must be shown later than this notification call because the panel needs to
   * observe its show/hide/close events.
   *
