...
#if defined(WITH_ACTION_CENTER_PLUGIN)
#include "../plugin-actioncenter/kmpactioncenter.h" // action center
extern void *loadPluginTranslation_actioncenter_helper;
#endif
 ...
 ************************************************/
Plugin::Plugin(const LXQt::PluginInfo &desktopFile, LXQt::Settings *settings,
               const QString &settingsGroup, LXQtPanel *panel)
    : QFrame(panel), mDesktopFile(desktopFile), mPluginLoader(0), mPlugin(0),
      mPluginWidget(0), mAlignment(AlignLeft), mPanel(panel) {
    ...
    bool found = false;
    if (ILXQtPanelPluginLibrary const *pluginLib =
            findStaticPlugin(desktopFile.id())) {
        qInfo() << "Plugin name: " << desktopFile.id() << endl;
        qInfo() << "this is called from static library" << endl;
    ...
    if (!isLoaded()) {
        if (!found)
            qWarning()
                    << QStringLiteral("Plugin %1 not found in the").arg(desktopFile.id())
                    << dirs;
    ...
    setProperty("NeedsHandle",
                mPlugin->flags().testFlag(ILXQtPanelPlugin::NeedsHandle));
    ...
    if (s.isEmpty()) {
        mAlignment =
                (mPlugin->flags().testFlag(ILXQtPanelPlugin::PreferRightAlignment))
                ? Plugin::AlignRight
                : Plugin::AlignLeft;
    } else {
        mAlignment = (s.toUpper() == QLatin1String("RIGHT")) ? Plugin::AlignRight
                                                             : Plugin::AlignLeft;
    }
    ...
    connect(mSettings, &PluginSettings::settingsChanged, this,
...
// NOTE2: we need to reference some (dummy) symbol from (autogenerated)
...
static plugin_tuple_t const static_plugins[] = {
    #if defined(WITH_ACTION_CENTER_PLUGIN)
    std::make_tuple(QLatin1String("actioncenter"),
    plugin_ptr_t{new KMPActionCenterPluginLibrary},
    loadPluginTranslation_actioncenter_helper),
    #endif
    //#if defined(WITH_DESKTOPSWITCH_PLUGIN)
    //        std::make_tuple(QLatin1String("desktopswitch"), plugin_ptr_t{new
    ...
    #endif
    //#if defined(WITH_QUICKLAUNCH_PLUGIN)
    //        std::make_tuple(QLatin1String("quicklaunch"), plugin_ptr_t{new
    //        LXQtQuickLaunchPluginLibrary},
    //        loadPluginTranslation_quicklaunch_helper),// quicklaunch
    //#endif
    //#if defined(WITH_SHOWDESKTOP_PLUGIN)
    //        std::make_tuple(QLatin1String("showdesktop"), plugin_ptr_t{new
    //        ShowDesktopLibrary}, loadPluginTranslation_showdesktop_helper),//
    //        showdesktop
    //#endif
    //#if defined(WITH_SPACER_PLUGIN)
    //        std::make_tuple(QLatin1String("spacer"), plugin_ptr_t{new
    //        SpacerPluginLibrary}, loadPluginTranslation_spacer_helper),// spacer
    //#endif
    //#if defined(WITH_STATUSNOTIFIER_PLUGIN)
    //        std::make_tuple(QLatin1String("statusnotifier"), plugin_ptr_t{new
...
struct assert_helper {
    assert_helper() {
        Q_ASSERT(std::is_sorted(
                     plugins_begin, plugins_end,
                     [](plugin_tuple_t const &p1, plugin_tuple_t const &p2) -> bool {
...
ILXQtPanelPluginLibrary const *
Plugin::findStaticPlugin(const QString &libraryName) {
    ...
    plugin_tuple_t const *plugin = std::lower_bound(
                ...
                [](plugin_tuple_t const &plugin, QString const &name) -> bool {
            return std::get<0>(plugin) < name;
});
...
bool Plugin::loadLib(ILXQtPanelPluginLibrary const *pluginLib) {
    ...
    if (!mPlugin) {
        qWarning()
                << QStringLiteral(
                       "Can't load plugin \"%1\". Plugin can't build ILXQtPanelPlugin.")
                   .arg(mDesktopFile.id());
        return false;
    }
...
bool Plugin::loadModule(const QString &libraryName) {
    ...
    ILXQtPanelPluginLibrary *pluginLib =
            qobject_cast<ILXQtPanelPluginLibrary *>(obj);
    if (!pluginLib) {
        qWarning() << QStringLiteral("Can't load plugin \"%1\". Plugin is not a "
...
void Plugin::watchWidgets(QObject *const widget) {
    // the QWidget might not be fully constructed yet, but we can rely on the
    // isWidgetType()
 ...
 ************************************************/
void Plugin::settingsChanged() { mPlugin->settingsChanged(); }

/************************************************

 ************************************************/
void Plugin::saveSettings() {
    mSettings->setValue(QStringLiteral("alignment"),
                        (mAlignment == AlignLeft) ? QStringLiteral("Left")
...
QMenu *Plugin::popupMenu() const {
    ...
    if (mPlugin->flags().testFlag(ILXQtPanelPlugin::HaveConfigDialog)) {
        QAction *configAction =
                new QAction(XdgIcon::fromTheme(QLatin1String("preferences-other")),
    ...
    QAction *moveAction =
            new QAction(XdgIcon::fromTheme(QStringLiteral("transform-move")),
                        tr("Move \"%1\"").arg(name), menu);
    menu->addAction(moveAction);
    connect(moveAction, SIGNAL(triggered()), this, SIGNAL(startMove()));

    menu->addSeparator();

    //  qDebug() << "number of element: "<< std::tuple_size<plugin_tuple_t>()<<endl;
    if(mName == QStringLiteral("Action Center or Control Center")){
        qDebug() << "action remove closed successfully"<<endl;
        return menu;

    }
    QAction *removeAction =
            new QAction(XdgIcon::fromTheme(QLatin1String("list-remove")),
                        tr("Remove \"%1\"").arg(name), menu);
    menu->addAction(removeAction);
    connect(removeAction, SIGNAL(triggered()), this, SLOT(requestRemove()));
    qDebug() << "other widget ran"<<endl;
    return menu;
}
/************************************************

 ************************************************/
bool Plugin::isSeparate() const { return mPlugin->isSeparate(); }

/************************************************

 ************************************************/
bool Plugin::isExpandable() const { return mPlugin->isExpandable(); }
