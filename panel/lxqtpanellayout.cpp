...
LayoutItemInfo::LayoutItemInfo(QLayoutItem *layoutItem)
    : item(layoutItem), separate(false), expandable(false) {
  if (!item)
    return;
...
LayoutItemGrid::~LayoutItemGrid() { qDeleteAll(mItems); }
...
LXQtPanelLayout::LXQtPanelLayout(QWidget *parent)
    : QLayout(parent), mLeftGrid(new LayoutItemGrid()),
      mRightGrid(new LayoutItemGrid()), mPosition(ILXQtPanel::PositionBottom),
      mAnimate(false) {
 ...
 ************************************************/
void LXQtPanelLayout::globalIndexToLocal(int index, LayoutItemGrid **grid,
                                         int *gridIndex) {
...
void LXQtPanelLayout::moveItem(int from, int to, bool withAnimation) {
  if (from != to) {
    ...
    if (fromGrid == toGrid) {
      ...
      toGrid->addItem(item);
      // recalculate position because we removed from one and put to another
      // grid
      LayoutItemGrid *toGridAux = 0;
      globalIndexToLocal(to, &toGridAux, &toIdx);
      Q_ASSERT(toGrid == toGridAux); // grid must be the same (if not something
                                     // is wrong with our logic)
...
QSize LXQtPanelLayout::sizeHint() const {
  ...
  if (isHorizontal()) {
    return QSize(ls.width() + rs.width(), qMax(ls.height(), rs.height()));
  } else {
    return QSize(qMax(ls.width(), rs.width()), ls.height() + rs.height());
  }
}
 ...
 ************************************************/
void LXQtPanelLayout::setItemGeometry(QLayoutItem *item, const QRect &geometry,
                                      bool withAnimation) {
...
void LXQtPanelLayout::setGeometryHoriz(const QRect &geometry) {
  const bool visual_h_reversed =
      parentWidget() && parentWidget()->isRightToLeft();
  // Calc expFactor for expandable plugins like TaskBar.
  double expFactor;
  {
    int expWidth = mLeftGrid->expandableSize() + mRightGrid->expandableSize();
    int nonExpWidth =
        mLeftGrid->sizeHint().width() - mLeftGrid->expandableSize() +
        mRightGrid->sizeHint().width() - mRightGrid->expandableSize();
    expFactor =
        expWidth ? ((1.0 * geometry.width() - nonExpWidth) / expWidth) : 1;
  }
  ...
  const int height_remain = 0 < bh ? geometry.height() % baseLines.size() : 0;
  {
    ...
    for (auto i = baseLines.begin(), i_e = baseLines.end(); i_e != i;
         ++i, base += bh) {
      *i = base;
    }
  }
    ...
    for (int c = 0; c < mLeftGrid->usedColCount(); ++c) {
      ...
      if (info.item) {
        ...
        if (info.separate) {
          ...
          if (!info.item->expandingDirections().testFlag(
        ...
        if (visual_h_reversed)
          rect.moveLeft(geometry.left() + geometry.right() - rect.x() -
                        rect.width() + 1);
...
void LXQtPanelLayout::setGeometryVert(const QRect &geometry) {
  ...
  double expFactor;
  {
    ...
    int nonExpHeight =
        mLeftGrid->sizeHint().height() - mLeftGrid->expandableSize() +
        mRightGrid->sizeHint().height() - mRightGrid->expandableSize();
    expFactor =
        expHeight ? ((1.0 * geometry.height() - nonExpHeight) / expHeight) : 1;
  }
 ...
 ************************************************/
int LXQtPanelLayout::lineCount() const { return mLeftGrid->colCount(); }
 ...
 ************************************************/
int LXQtPanelLayout::lineSize() const { return mLeftGrid->lineSize(); }
...
void LXQtPanelLayout::finishMovePlugin() {
  PluginMoveProcessor *moveProcessor =
      qobject_cast<PluginMoveProcessor *>(sender());
  if (moveProcessor) {
    Plugin *plugin = moveProcessor->plugin();
    int n = indexOf(plugin);
    plugin->setAlignment(n < mLeftGrid->count() ? Plugin::AlignLeft
                                                : Plugin::AlignRight);
