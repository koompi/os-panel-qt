...
WizardAgent::WizardAgent(QObject *parent)
    : BluezQt::Agent(parent), m_fromDatabase(false) {}
...
bool WizardAgent::isFromDatabase() { return m_fromDatabase; }
...
QString WizardAgent::getPin(BluezQt::DevicePtr device) {
  ...
  const QString &xmlPath = QStandardPaths::locate(
      QStandardPaths::DataLocation, QStringLiteral("pin-code-database.xml"));
  ...
  while (!xml.atEnd()) {
    ...
    if (attr.hasAttribute(QLatin1String("type")) &&
        attr.value(QLatin1String("type")) != QLatin1String("any")) {
    ...
    if (attr.hasAttribute(QLatin1String("oui"))) {
      if (!device->address().startsWith(
              attr.value(QLatin1String("oui")).toString())) {
        continue;
      }
    }
...
void WizardAgent::requestPinCode(BluezQt::DevicePtr device,
                                 const BluezQt::Request<QString> &req) {
...
void WizardAgent::displayPinCode(BluezQt::DevicePtr device,
...
void WizardAgent::requestPasskey(BluezQt::DevicePtr device,
                                 const BluezQt::Request<quint32> &req) {
...
void WizardAgent::displayPasskey(BluezQt::DevicePtr device,
...
void WizardAgent::requestConfirmation(BluezQt::DevicePtr device,
                                      const QString &passkey,
